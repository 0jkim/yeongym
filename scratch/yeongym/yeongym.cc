...
int main(int argc, char *argv[])
{
    ...
    CommandLine cmd;
    cmd.AddValue("centralFrequencyBand1", "The system frequency to be used in band1", centralFrequencyBand1);
    ...
    mobility.SetPositionAllocator("ns3::GridPositionAllocator","MinX",DoubleValue(-isd/2),"MinY", DoubleValue(-isd/2),"GridWidth", UintegerValue(1), "LayoutType", StringValue("RowFirst"));
    ...
    {
        Ptr<ConstantVelocityMobilityModel> mob = gridScenario.GetUserTerminals().Get(i)->GetObject<ConstantVelocityMobilityModel>();
        if (!mob) 
        {
    ...
    CcBwpCreator::SimpleOperationBandConf bandConf1(centralFrequencyBand1, bandwidthBand1,1,BandwidthPartInfo::UMa_nLoS);
